...
public class WordToHtmlUtil {
        ...
        try {
            ...
            wordToHtmlConverter.setPicturesManager((imageStream, pictureType, name, width, height) -> {
                try {
                    ...
                    //boolean upload = FileUtils.upload(new FileInputStream(fileImage), qiNiuName);
                    return "上传后的图片地址";
                } catch (Exception e) {
    ...
    public static String docxToHtmlText(MultipartFile file) throws Exception {
        ...
        try {
            ...
            File imageDir = new File(imageDirStr);String[] imageList = imageDir.list();
            if (imageList != null) {
                for (int i = 0; i < imageList.length; i++) {
                    try {
                        ...
                        if (fileImage.exists()) {
                            ...
                            //此处上传到自己的文件服务器
                            String qiNiuName = "";//文件名
                            boolean upload = false;//FileUtil.upload(new FileInputStream(fileImage), qiNiuName);
                            if (!upload) {continue;}
                            else {
                                //修改文档中的图片信息
    ...
    public static void main(String[] args) {
        try {
            String content = docxToHtmlText(getMulFileByPath("C:\\Users\\Administrator\\Desktop\\hah.docx"));
            System.out.println(content);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    /*** 获取MultipartFile文件** @param picPath* @return*/
    private static MultipartFile getMulFileByPath(String picPath) {
        FileItem fileItem = createFileItem(picPath);
        MultipartFile mfile = new CommonsMultipartFile(fileItem);return mfile;
    }
